...
- **`/ui`** (folded into `/implement` as the pre-code step, also standalone) designs the **visual** against `StrandDesign`: token-by-token mapping plus an **HTML preview per state (`show_widget`, faithful to StrandPalette)** the user approves *before* any code — HTML, not PNG, because that's what the user can actually see. Leans on `design-for-ai` + `lazyweb` + `impeccable`.
...
**Two risk lanes (how much process runs).** Not every change earns the same ceremony — running the full gate on a margin tweak costs more than the rework it prevents. `/pm` tags each issue with a **`Carril`** field, and `/implement`/`/ui`/`/qa` read it:
