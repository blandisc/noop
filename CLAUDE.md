...
Cénit is a fully **offline, on-device** companion app for WHOOP straps (4.0 / 5.0 / MG): it pairs over BLE, stores everything locally in SQLite, and computes recovery / strain / HRV / sleep on-device. No server, no account, no network. Not affiliated with WHOOP; not a medical device (see [DISCLAIMER.md](DISCLAIMER.md)).
