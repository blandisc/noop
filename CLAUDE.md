...
- `Packages/CenitDesign` — the SwiftUI design system (single source of visual truth).
...
- **The design system is law.** Screens use only `CenitDesign` tokens (`StrandPalette`, `StrandFont`, `CenitMetrics`) and components (`liquidGlass(_:)`, `StatTile`, … — see [docs/design-system/CATALOGO.md](docs/design-system/CATALOGO.md) for the generated rol→símbolo→archivo index). No raw hex, font sizes, spacing, or one-off cards. If a token is missing, add it to `CenitDesign` (with a `#Preview`) — don't inline it. **The enforcement contract — gate matrix, exemption taxonomy (`// token-exempt(<categoria>): …`), the ×3 rule, the legal baseline-raise path — is [docs/design-system/CONTRATO.md](docs/design-system/CONTRATO.md)**; CI validates the tree against it. The **DNA** that gives those tokens a point of view lives in [docs/design-system/DESIGN.md](docs/design-system/DESIGN.md): **«Liquid Glass · El Eje»** (tinted glass on a white canvas, two regimes — mosaico / sobrio; see also [LIQUID-GLASS.md](docs/design-system/LIQUID-GLASS.md)) is **canonical** for new/redesigned screens. «Instrumento diurno / papel cálido» is the previous generation (absorbed, in migration — `DESIGN.md` §8). The dark system is **legacy** (maintain, don't extend; Watch OLED is the only live exception). For iOS-native decisions (HIG, SF Symbols, motion) cite Apple via the **Cupertino** MCP, not guesswork.
...
- **Finish the job, then clean up.** When QA passes, take the work all the way: merge, **close the issue at Done in the same step you delete the branch** (not as a later chore — three issues sat green-but-stale for up to 26 days because it was), delete the branch, run `Tools/cleanup.sh --apply`, leave the tree clean — no half-done branches or stray open PRs. **Then sync the canonical checkout** so it always matches `origin/iOS`: `git -C ~/code/noop fetch origin && git -C ~/code/noop merge --ff-only origin/iOS` (`--ff-only` preserves any uncommitted work there; if it can't fast-forward, tell the user instead of forcing). **"Production" — and the end of the flow — means merged to `origin/iOS` **and** pulled into `~/code/noop`.** Building/installing on the iPhone from Xcode is **optional and the owner's alone**: it is *not* a step of the flow, never a gate, never automated or scheduled (DECISIONS.md 2026-08-30). Don't tack it onto the end of a task or treat a change as unfinished until it's on the phone.
