...
The flow is **requirement → (architecture) → experience → screen → code → QA**, with acceptance criteria as the through-line. Six skills cover it (the two design ones only kick in for screen work; the architecture one only for structural/deep changes; QA gates every change before merge):
