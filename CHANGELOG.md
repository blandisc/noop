...
- **La gráfica de etapas de sueño se lee de un vistazo / The sleep stages graph reads at a glance.**
  **ES** — El hipnograma de **Sueño** se rediseña: los rótulos del eje («Despierto», «REM», «Ligero», «Profundo») ya no se parten en dos líneas —caben en una sola, alineados al dato—; las bandas son **más gruesas** y la noche se lee como una **escalera continua** en vez de píldoras sueltas; las líneas-guía dejan de ser una reja (ahora un punteado tenue) y los conectores grises bajan a un hilo fino que no compite con el color; y aparece un **eje de tiempo** abajo (de la hora en que te dormiste a la de despertar). Solo presentación.
  **EN** — The **Sleep** hypnogram is redesigned: the axis labels ("Awake", "REM", "Light", "Deep") no longer wrap onto two lines — they fit on one, aligned to the data; the bands are **thicker** and the night reads as a **continuous staircase** instead of scattered pills; the guide lines stop being a grid (now a faint dotted anchor) and the gray connectors drop to a thin hairline that no longer competes with the color; and a **time axis** appears below (from when you fell asleep to when you woke). Presentation only.
  ([Packages/StrandDesign/Sources/StrandDesign/Hypnogram.swift](Packages/StrandDesign/Sources/StrandDesign/Hypnogram.swift))
