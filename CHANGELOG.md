...
- **Hoy cabe completa en una pantalla, y el número del veredicto queda centrado / Today fits on one screen, and the verdict number is centered.**
  **ES** — La pantalla **Hoy** se desbordaba mientras tu base apenas se afina (los primeros días, «N de 14 noches»): **Pasos** y **Estrés** quedaban tapados por la barra de abajo. La apretamos para que **entre completa en una pantalla**: el reloj del veredicto es un poco más chico, el **«/100» ahora va centrado justo debajo del número** (antes colgaba a la derecha, descuadrado), la línea «Afinando con tu strap» ocupa menos, y las tarjetas de «Métricas de hoy» son un pelín más bajas. En iPhone grandes ya no hay que hacer scroll; en los más chicos puede quedar un mínimo.
  **EN** — The **Today** screen overflowed while your baseline is still settling (the first days, "N of 14 nights"): **Steps** and **Stress** got hidden behind the bottom bar. We tightened it so it **fits on one screen**: the verdict clock is a bit smaller, the **"/100" now sits centered right under the number** (it used to hang off to the right, off-center), the "Tuning with your strap" line takes less room, and the "Today's metrics" cards are slightly shorter. On larger iPhones there's no more scrolling; on the smallest ones a tiny bit may remain.
