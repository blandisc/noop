...
- **Mapa muscular: la respuesta lidera, con muñeco anatómico / Muscle map: the answer leads, with an anatomical figure.**
  **ES** — La hoja **«Qué entrenar hoy»** se reordenó para que **la recomendación vaya primero**: un *hero* arriba con los músculos **frescos para entrenar** y el **gate de recuperación anclado** (una barra de bandas **baja · base · despejada** con un marcador en tu score, para entender *por qué* hoy estás despejado), más la línea **«Sigue cargado hoy: …»**. El mapa pasó de manchas esquemáticas a **siluetas anatómicas** de frente y espalda, con un **rótulo flotante** que nombra tu músculo más cargado. El segmentado 3/7/14 d se renombró **«Lente · recencia del tinte»** (la banda semanal sigue fija a 7 d) y se añadió **«Ver el método»** al pie. La matemática no cambió (modelo `MuscleFatigueMap`, citado).
  **EN** — The **"What to train today"** sheet is reordered so **the recommendation leads**: a hero up top with the muscles that are **fresh to train** and the **recovery gate anchored** (a banded **low · base · clear** bar with a marker at your score, so you see *why* you're clear today), plus a **"Still loaded today: …"** line. The figures went from schematic blobs to **detailed front/back anatomical silhouettes** with a **floating label** naming your most-loaded muscle. The 3/7/14-day segment is relabeled **"Lens · tint recency"** (the weekly band stays fixed at 7 d) and a **"See the method"** disclosure was added at the foot. The math is unchanged (the cited `MuscleFatigueMap` model).
