...
- **Las tarjetas de Hoy se abren con una animación propia / Today's cards open with their own animation.**
  **ES** — En **Hoy**, al tocar una tarjeta de «Métricas de hoy» (o el número grande de recuperación), ahora **la propia tarjeta se expande** hasta volverse su detalle, y al cerrar **regresa a su lugar** — en vez de que el detalle suba desde abajo como una pantalla aparte. Al tocar, la tarjeta **se eleva** un instante hacia tu dedo (ese realce es el primer cuadro de la apertura), sin ningún ícono ni adorno extra en reposo. Es una apertura **exclusiva de Hoy**: la misma métrica abierta desde **Cuerpo** se comporta como siempre. (Requiere iOS 18 o más; en versiones anteriores abre como antes.)
  **EN** — On **Today**, tapping a "Today's metrics" card (or the big recovery number) now makes the **card itself expand** into its detail and **collapse back into place** when you close it — instead of the detail sliding up as a separate screen. On touch, the card **lifts** toward your finger for an instant (that lift is the first frame of the open), with no extra icon or chrome at rest. It's a **Today-only** opening: the same metric opened from **Body** behaves as before. (Requires iOS 18+; earlier versions open as before.)
