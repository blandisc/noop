...
- **Hoy: el dock de puntos del pager baja al ras de la barra / Today: the pager dots dock drops to sit just above the tab bar.**
  **ES** — En **Hoy**, los puntitos del pager (el indicador de página) flotaban con demasiado aire sobre la barra de pestañas. Ahora bajan hasta quedar **al ras**, flotando ~3pt sobre la barra: el capsule se bottom-alinea dentro de su área tocable de 28pt (se conserva el toque) y la «Barra de instrumento» pierde 4pt de padding superior —sus iconos suben un poco, en las 5 pestañas—. El sobrante vertical se **reubica arriba** del dock, no se elimina. Sin cambios en el dial, el encabezado ni la rejilla de métricas.
  **EN** — On **Hoy**, the pager dots (the page indicator) floated with too much air above the tab bar. They now drop **flush**, floating ~3pt above the bar: the capsule bottom-aligns within its 28pt tap target (tap area preserved) and the instrument bar loses 4pt of top padding —its icons rise a touch, across all five tabs—. The freed space **moves above** the dock rather than being removed. No changes to the dial, header, or metrics grid.
  ([Cenit/Screens/TodayView.swift](Cenit/Screens/TodayView.swift), [CenitApp/App/InstrumentTabBar.swift](CenitApp/App/InstrumentTabBar.swift))
