...
//
//   1. Pair (dose_d, outcome_{d+1}) for each logged day with a next-day outcome — a single
//      FIXED lag L=1 (same next-day alignment ActivityCostEngine uses, via
//      CorrelationEngine.shiftDay). No lag SEARCH happens here, so this engine introduces no
//      multiple-comparison problem of its own (that guard belongs to any lag-search caller,
//      which must run one Benjamini-Hochberg family — see MultipleComparisons).
//   2. Fit the PERSONAL slope β_user = OLS slope of outcome on dose via the slope that
//      CorrelationEngine.pearson already returns (exact Student-t p, NOT a normal
//      approximation); n_user = paired days. pearson needs ≥ 3 pairs AND spread in both
//      axes; if it can't fit, β_user is undefined → pure prior.
...
public struct DoseResponse: Equatable, Sendable {
    ...
    /// Curve points (dose, projectedOutcomeDelta-from-baseline) for the chart, dose 0…maxCurveDose,
...
public enum DoseResponseEngine {
    ...
    public static func estimate(behavior: DosedBehavior,
        ...
        // Pair each logged dose day D with the NEXT-day outcome (D+1) — the fixed L=1 alignment.
        ...
        // This is OUR CorrelationEngine.pearson — exact Student-t inference, not the normalCDF
        // approximation the upstream twin once used.
