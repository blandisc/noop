...
/// day (a drive, a workout that's all arms, a phone left at home) can't drag the whole calibration: a busy
/// 15,000-step day pins the ratio far more reliably than a near-still 500-step day (more footfalls ⇒ less
/// ratio noise), so motion VOLUME drives the fit instead of every day counting equally. A user with no
/// phone step history to fit against can set `k` manually with the calibration slider.
///
/// Pure value type — no I/O, no clock reads, fully unit-tested. Ported from the upstream
/// StepsEstimateEngine (NoopApp/noop); the calibration-status copy lives in the app layer so it can be
/// localized (es/en) — the engine only reports the state.
public enum StepsEstimateEngine {
    ...
    /// drive the fit instead of every day counting equally. Each day's ratio carries weight = its motion
    ...
    /// Weighted median of `xs` with per-element `weights`. Sort by value, walk the cumulative weight,
