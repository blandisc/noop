...
// gate + the live-path de-dup, folded into one pure, deterministic, DB-free engine. The app layer
// (InactivityPrefs + the BLE offload hook) supplies the config/state and fires the buzz.
//
...
/// caller stores this verbatim (the InactivityPrefs LAST_* keys) and feeds the prior value back into
/// the next `evaluate`. A fresh user starts from `.initial`.
