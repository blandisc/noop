...
/// R-R series (no trig, no platform-dependent floats), so every run reproduces
/// byte-identical inputs and therefore identical `RhythmRegularity` labels and rounded
/// stats — a stable regression gate independent of float drift.
///
...
final class RhythmScreenerTests: XCTestCase {
    ...
    /// A tiny deterministic LCG (Numerical Recipes constants) over UInt32, producing a
    /// fixed sequence. `next(mod:)` returns an Int in [0, mod).
    struct LCG {
        var state: UInt32
