...
/// DoseResponseEngine — per-user dose slope shrunk toward a population prior. Exercises the
/// cold-start (pure prior), the exact shrinkage weight, personal-slope recovery at large n,
/// the person-overrides-prior flip, the clamp, and the curve/delta helpers.
final class DoseResponseEngineTests: XCTestCase {
    ...
    func testPerUnitIsClampedToPriorRange() {
        ...
        for k in 0..<40 {
            let dose = k % 4
            // Space anchors so D+1 never collides: anchor every 3 days across wide months.
