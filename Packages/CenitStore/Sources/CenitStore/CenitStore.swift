...
public enum CenitStoreInfo {
    ...
    public static var latestMigration: String { CenitStore.makeMigrator().migrations.last ?? "v0" }
}

/// CenitStore is an `actor`: its public API is `async`, and all GRDB work runs on the
...
public actor CenitStore {
    ...
    private init(dbWriter: any DatabaseWriter) throws {
        ...
        try CenitStore.makeMigrator().migrate(dbWriter)
    }
    ...
    public static func inMemory() async throws -> CenitStore {
        try CenitStore(dbWriter: try DatabaseQueue())
    }
