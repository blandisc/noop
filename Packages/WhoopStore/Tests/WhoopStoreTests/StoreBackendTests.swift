...
final class StoreBackendTests: XCTestCase {
    ...
    func testPoolInitRunsMigrations() async throws {
        ...
        let store = try await WhoopStore(path: path, backend: .pool(maxReaders: 2))
    ...
    func testPoolBackendMatchesQueueBackendFieldByField() async throws {
        ...
        let queueStore = try await WhoopStore(path: queuePath)                       // default .queue
        let poolStore = try await WhoopStore(path: poolPath, backend: .pool(maxReaders: 2))
