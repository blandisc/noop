...
struct TodayView: View {
    ...
    private func loadAll() async {
        ...
        recomputeDerived()
        // Issue every query concurrently, then collect — instead of serial awaits that each suspend
        // back to the main actor before issuing the next. The store is a serial DatabaseQueue so I/O
        // still serializes, but the memoized ensureStore() makes the parallel first-callers share ONE
        // open, and the queries run back-to-back with no main-actor ping-pong.
