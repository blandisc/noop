...
/// Append-only on-disk archive of HISTORICAL_DATA record frames that FAILED decode (FER-693, #77 / #91).
///
/// The strap frees acked history the instant we send the trim ack. On an unmapped firmware layout the
/// Backfiller decodes those records to zero rows, yet still has to ack the trim (refusing would wedge the
/// whole offload on a re-send loop). Without somewhere to put the raw bytes first, every undecodable
/// record is gone forever while the UI shows a healthy "History synced". This archive is the user's only
/// remaining copy — and the corpus a later layout mapping re-ingests (e.g. WHOOP 4.0 v25).
///
/// Format: newline-delimited JSON, one object per line, fsynced before returning so the bytes are durable
/// BEFORE the trim ack deletes the strap's copy (the whole point):
...
struct RawHistoryArchive {
    ...
    /// lines to make room rather than refusing the write — but only down to a per-version retention floor
    /// (see `perVersionFloor`), so a brand-new layout version is never binned merely because common
    /// versions filled the file. Only when the incoming frames alone can't fit even an empty archive does
    /// `archive` skip them (reported as `.capReached`). (#344)
    static let maxBytes = 5 * 1024 * 1024

    /// Per distinct layout VERSION, keep at least this many of the newest archived lines, immune to cap
    /// eviction. A never-seen version (WHOOP 4 v19, WHOOP 5 v20/v21 — the `hist_version` byte at
    /// `frame[5]` / `frame[9]`) emits only a handful of frames, so this floor guarantees those rare
    /// samples survive while the most-populous versions shed their oldest surplus. Bounded: the floor can
    /// hold at most `floor × distinctVersions` lines, a tiny set. (#344)
    ...
    enum Result: Equatable {
        ...
        /// The archive is full; nothing was written but the caller may still ack. `count` frames were
        /// dropped (counted as unarchived so the sync status never claims "saved").
        case capReached(count: Int)
        /// The write genuinely failed. The caller must NOT ack — hold the cursor so the strap re-sends
        /// the chunk (no data loss either way).
        case failed
    }
    ...
    /// Default location: `<AppSupport>/OpenWhoop/` (alongside the SQLite store), created on demand.
    ...
    init(directory: URL? = nil,
        ...
        if let directory {
            ...
            self.directory = base.appendingPathComponent("OpenWhoop", isDirectory: true)
        }
    ...
    /// A per-version retention key. Family is part of the key so a WHOOP 4 v18 and a WHOOP 5 v18 are kept
    /// as distinct buckets (their layouts differ despite the shared version number).
    private struct VersionKey: Hashable { let family: String; let version: Int }

    /// Durably append `frames` as JSONL. `trim`/`family` tag each line so the corpus is replayable. Empty
    /// input is a no-op success. See `Result` for the ack contract.
    ///
    /// Cap handling (#344): if appending would exceed `maxBytes`, oldest SURPLUS lines are evicted
    /// (oldest-first, from the most-populous versions) — but never below `perVersionFloor` newest lines of
    /// any distinct version, so a rare never-seen layout is never binned just because common versions
    /// filled the archive. The file is rewritten atomically in that case. Only when the incoming frames
    /// alone can't fit even an empty archive are they skipped (`.capReached`).
    func archive(_ frames: [[UInt8]], trim: UInt32, family: DeviceFamily) -> Result {
        ...
        // The incoming batch alone can't fit even an empty archive: nothing we can evict makes room, so
        // skip it (the offload may still ack — by now there is ample sample material). Preserves the prior
        // cap contract for this degenerate case.
        if incomingBytes > maxBytes {
            return .capReached(count: frames.count)
        }

        // Over cap: rewrite with floor-aware eviction. Read existing lines (oldest first), append the new
        // lines (which sort newest), then drop oldest surplus until we fit — never dropping a line within
        // the newest `perVersionFloor` of its version. `evictLines` is the pure, unit-tested core.
    ...
    /// The `VersionKey` of one archived JSONL line, re-derived from its stored frame + family; `nil` for a
    ...
    private static func lineVersionKey(_ line: String) -> VersionKey? {
        guard let frame = frameAndFamily(line) else { return nil }
        return VersionKey(family: frame.family.rawValue,
                          version: RawHistoryArchive.versionByte(frame.bytes, family: frame.family))
    }

    /// Hand-parse one JSONL line into its stored frame bytes + family (the only dynamic fields are
    /// `family` and the [0-9a-f] `frameHex`). `nil` for a malformed line.
    private static func frameAndFamily(_ line: some StringProtocol) -> (bytes: [UInt8], family: DeviceFamily)? {
    ...
    /// Floor-aware cap eviction (#344), PURE so it is unit-testable. `lines` is the full newline-terminated
    /// JSONL set, oldest-first (existing + incoming). Drops oldest SURPLUS lines until the UTF-8 byte total
    /// fits `maxBytes`, but NEVER a line within the newest `floor` lines of its version — so a rare
    /// never-seen layout (WHOOP 4 v19, WHOOP 5 v20/v21) always survives even when common versions fill the
    /// archive. Bounded: if everything left is floor-protected it stops even if still over cap (the floor
    /// wins — `floor × distinctVersions` is tiny).
    ...
    /// `archive` writes. Malformed lines are skipped; an absent/empty file yields [].
    func readAll() -> [(frame: [UInt8], family: DeviceFamily)] {
        guard let text = try? String(contentsOf: fileURL, encoding: .utf8) else { return [] }
        return text.split(separator: "\n").compactMap { line in
            RawHistoryArchive.frameAndFamily(line).map { ($0.bytes, $0.family) }
        }
    }
    ...
    /// (deviceId, ts), so a re-run can't double-insert. Returns rows recovered (for logging). Throws if a
    /// store insert fails — the caller MUST NOT advance the replay gate in that case, or these records
    /// (whose only surviving copy is this archive) would never be retried. (#152)
    @discardableResult
    func replay(into store: BackfillStoreWriting, deviceId: String) async throws -> Int {
        ...
        for family in Set(archived.map(\.family)) {
            ...
            // replays every release, and dedupe makes those re-runs insert 0 — counting decoded rows would
            // log a false "retro-decoded N" success on every update. (#152)
