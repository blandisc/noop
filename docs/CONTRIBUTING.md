...
**iOS app (`NOOPiOS`) is the app target**; `Strand/` is its shared app layer. The packages reuse the
...
| CoreBluetooth, bonding, offload, live state | `Strand/BLE`, `Strand/Collect` | App layer — wraps the pure packages. |
| A screen or navigation destination | `Strand/Screens`, `Strand/System` | App layer. |
...
The scheme is `NOOPiOS`; the built product is `NOOP.app` (`project.yml` sets `PRODUCT_NAME: NOOP`,
bundle id `com.noopapp.noop`). A **free** Apple ID is enough to install a personal build on your own
...
rather than rewrites of the core. Today the **iOS app (`NOOPiOS`) is the shipping app**; the items
