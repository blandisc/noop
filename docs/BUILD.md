...
├── Cenit/                     # SwiftUI app layer (built by Cenit; module/product: Cenit, display name NOOP)
...
- The Xcode **scheme is `Cenit`**, and the built product is **`Cenit.app`** — `project.yml` sets
  `PRODUCT_NAME: Cenit` and bundle id `com.noopapp.noop`, with display name `NOOP`.
...
NOOP connects over CoreBluetooth (`Cenit/BLE/BLEManager.swift`). The central manager is created
on the main queue:
