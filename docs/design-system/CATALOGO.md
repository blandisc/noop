...
| Badge de procedencia (Instrumento) | `SourceBadge` | `Components.swift` | Legado — solo mientras quede un consumidor Instrumento; preferir `LiquidOrigenBadge`. | No en pantallas Liquid nuevas (usa `LiquidOrigenBadge`); no en pie de hoja (usa `LiquidOrigenChip`). |
