...
| Menú «···» | `LiquidMenu` | `LiquidGlass/LiquidMenu.swift` | El menú contextual de un «···» en pantalla Liquid: filas con icono, subtítulo de estado, destructiva al final y un nivel de submenú. | No en pantallas aún Instrumento (usa `PaperMenu` hasta que migren); no para una acción única (usa un botón). |
