# Assets

Brand and icon assets for the Strand / NOOP design system. Copied from the app's
asset catalogs and `docs/assets/`.

## `brand/`

| File | Notes |
|---|---|
| `logo.svg` | App logo (vector) |
| `banner.svg` | README / marketing banner (vector) |

## `app-icon/`

| File | Size | Source |
|---|---|---|
| `icon_1024.png` | 1024×1024 | iOS marketing icon (`StrandiOS` catalog) |
| `icon_512x512.png` / `@2x` | 512 / 1024 | macOS (`Strand` catalog) |
| `icon_256x256.png` / `@2x` | 256 / 512 | macOS |
| `icon_128x128.png` / `@2x` | 128 / 256 | macOS |
| `icon_32x32.png` / `@2x` | 32 / 64 | macOS |
| `icon_16x16.png` / `@2x` | 16 / 32 | macOS |

> **Canonical sources** (don't edit these copies — edit the catalog, then re-copy):
> - iOS icon: `StrandiOS/Resources/Assets.xcassets/AppIcon.appiconset/`
> - macOS icon: `Strand/Resources/Assets.xcassets/AppIcon.appiconset/`
> - Brand SVGs: `docs/assets/`
