...
**Nota — pull-to-refresh para sincronizar (iOS, FER-204):** el `ScrollView` del `iosBody` lleva `.refreshable`; jalar hacia abajo fuerza un sync de la banda y dispara una háptica `.impact(weight: .medium)` al provocarse (vía `.sensoryFeedback` sobre el contador `syncHaptic`). El indicador de carga es el **nativo** de `.refreshable`, no una animación propia. La acción (`pullToSync`) ramifica por estado: conectada → `syncNow()` (offload `.manual`, sin rate-limit); banda conocida pero desconectada (`lastSyncedAt != nil`) → `scan()`, y el handshake de reconexión auto-dispara el sync (`requestSync(.connect)`); sin banda conocida → solo `repo.refresh()` local, sin escaneo ni error. El spinner se retira pronto (~1.2 s) y NO espera al offload largo, que sigue reflejándose en `syncMeta`; al llegar datos los scores se recalculan solos vía `repo.refreshSeq`. No toca el protocolo BLE — reusa `syncNow()`/`scan()` existentes.
...
**Nota — la apertura de las tarjetas es un zoom-morph, sólo en Hoy (iOS, FER-210 · supersede el «pulsado vía `TileButtonStyle` overlay» de FER-180):** tocar un **tile** de «Métricas de hoy» o el **numeral de recuperación** del héroe ya no abre el detalle con una hoja genérica que sube: la **tarjeta se expande** en su detalle y **colapsa de regreso** al cerrar (`matchedTransitionSource(id:in:)` en las **9 superficies** —8 tiles + numeral— + `.navigationTransition(.zoom(sourceID:in:))` en las **dos** hojas de detalle de Hoy, `MetricInfoSheet` y `MetricDetailScreen`, con un `@Namespace cardZoom`; el `id` empata el de cada hoja: `recovery`/`strain`/`sleep`/`heart_rate`/`spo2`/`steps`/`stress` por `MetricInfo.id`, `hrv`/`rhr` por `descriptor.key`). Gateado a **iOS 18+** vía los helpers `cardZoomSource`/`cardZoomDestination` (en iOS 17 / no-iOS es no-op → hoja normal sin morph). Es **exclusivo de Hoy**: el cableado vive en `TodayView`, así que la misma métrica abierta desde **`CuerpoView`** (mismo `MetricDetailScreen`) conserva su apertura **sin morph** — son distintas a propósito. El `TileButtonStyle` cambió de **darken pressed** (overlay `ink.opacity(0.05)`, FER-180) a **realce al pulsar**: `scaleEffect(1.03)` + borde `hairline → hairlineStrong`, sin sombra — la tarjeta «se eleva» como primer cuadro del morph, sin marca permanente en reposo.
