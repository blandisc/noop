...
**Nota — la frescura de sync baja al pie de la rejilla (iOS, FER-294/FER-305 · mueve FER-278/265):** el renglón `SyncInline` (glifo `arrow.triangle.2.circlepath` + «hace X» en reposo / glifo girando + «N paquetes» en backfill) **se sacó del `headerBlock`** y se montó **abajo a la derecha** (bajo el tile de Estrés) como **`.overlay(alignment: .bottomTrailing)`** del contenido del scroll —igual que la pista del chevron arriba—, así **NO consume alto de layout** y Hoy conserva su presupuesto vertical de una pantalla (FER-217). Montarla como fila en flujo (intento inicial de FER-294) rebasaba el presupuesto y la «Barra de instrumento» la cortaba (FER-305). El encabezado conserva **fecha (izquierda) + batería del strap (derecha)** — la batería queda como única señal de estado arriba. La acción accesible «Sincronizar» sigue en el `headerBlock` (`accessibilityAction`). Revierte la consolidación sync+batería del header de FER-278.
