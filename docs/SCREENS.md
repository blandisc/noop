...
**Nota — la frescura de sync baja al pie de la rejilla (iOS, FER-294 · mueve FER-278/265):** el renglón `SyncInline` (glifo `arrow.triangle.2.circlepath` + «hace X» en reposo / glifo girando + «N paquetes» en backfill) **se sacó del `headerBlock`** y se montó al **pie de `iosMetricsSection`**, alineado a la derecha (bajo el tile de Estrés), apilado bajo el nudge «Conectar Apple Salud» cuando éste aparece. El encabezado conserva **fecha (izquierda) + batería del strap (derecha)** — la batería queda como única señal de estado arriba. La acción accesible «Sincronizar» sigue en el `headerBlock` (`accessibilityAction`). Revierte la consolidación sync+batería del header de FER-278.
