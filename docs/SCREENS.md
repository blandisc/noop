# Cénit iOS — Inventario de Pantallas
...
Corre el UI test `CenitScreenshotTests`, copia las PNGs al repo y sube la fecha del toolbar. Luego commitea los cambios en `docs/`.
...
CuerpoView  → RecoveryDetailScreen (sheet «Instrumento»: Recuperación — FER-225) ·
             ...
             StressDetailScreen (sheet claro «Instrumento»: Estrés — valor de hoy + bandas universales + qué lo mueve + ⓘ por concepto — FER-241; **«Estrés a lo largo del día»**: el «mapa del día» = carril vertical con la curva intradía cruzada con el calendario, EventKit on-device + `StressEngine`, `StressDayMapBlock` — FER-377; **«Tus patrones»**: una línea observacional de estrés por momento del día (`StressTimeOfDayPatterns`) y por **evento recurrente del calendario** (`StressEventPatterns`, una sola lectura EventKit on-device, sin persistir — FER-388) + «Explóralo en el Coach» que salta a la pestaña Coach vía `TabRouter` — FER-378) ·
...
**Descripción:** Landing curado de la capa «historia / entre-días» (pestaña **Cuerpo**, FER-186; rediseño a **tarjetas por dominio**). Papel claro «Instrumento» (color solo en el dato): el `body` es una columna de **tarjetas de dominio** (`theme.surface` + hairline, `cardRadius` 16) — ya NO una lista plana de `MetricRow`. Orden vertical: **Título + fecha** → **Recuperación** (tarjeta héroe, el único numeral dominante `instrumentoHero(56)` + tendencia 14d) → **Descanso & carga** / **Vitales** / **Actividad** / **Longevidad** → nudge de conectar → acciones al pie. Cada **stat** dentro de una tarjeta es su propio destino al detalle (el atajo directo que tenían las filas); la **cabecera es una etiqueta quieta sin chevron** — «Ver todas las métricas» al pie es la única puerta al catálogo (sin duplicar destino). Solo el héroe lleva tendencia en el landing (y el único sparkline calculado); los stats densos son un número (label · valor en su color de dato · leyenda opcional), sin sparkline. El héroe atenúa al tocar igual que los stats (`CardPressStyle`, ink 5% / easeOut 0.12).
...
**Componentes:** `Explainer Card (pesos: HRV 60% / RHR 20% / sleep 15% / resp 5%)`, `Day Cards (Recovery/Strain/Sleep + Cénit badge)`, `Toolbar Recompute`
...
**Componentes:** `RecoveryBand` (compartida), `LiveWorkoutHubRow` (En vivo), `QuietButton`, `TrainingSoonSheet`, tokens Instrumento (`theme.surface`/`hairline`/`ink`, `CenitMetrics`).
