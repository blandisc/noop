...
- **Honesty of power (load-bearing for the copy):** by Noether (1987), detecting even a *large* effect (P(X>Y)=0.75) at 80% power, α = 0.05, needs ≈ 21 days per arm; a 21-day window typically yields 5–10/arm → real power ~20–40%. So **«no se ve» will be frequent and the copy must say «todavía»**, never «no existe» — abstention over noisy discovery (the Welltory/Oura Discoveries failure mode). Verdicts map to the shipped `Verdict` rawValues: `.sustained` = «probable efecto (en ti)», `.notSustained` = «no se ve (todavía)», `.insufficient` = «datos insuficientes».

References: **Wilcoxon 1945** (*Biometrics Bulletin* 1(6):80); **Mann & Whitney 1947** (*Ann Math Statist* 18(1):50); **Hodges & Lehmann 1963** (*Ann Math Statist* 34(2):598); **Lehmann 1975** (*Nonparametrics*, Holden-Day); **Noether 1987** (*JASA* 82(398):645); N-of-1 framing **Lillie et al. 2011** (*Per Med* 8(2):161). APPROXIMATE, no clinical claim.

#### Claims allow-list — N-of-1 tag experiment (PR-blocking)

**Allowed:** «probable efecto (en ti)» · «no se ve (todavía)» · «datos insuficientes» · arm counts («5 días con · 7 sin») · «~X ms/min de diferencia típica» (Hodges-Lehmann, always with «~») · «se excluyeron N días (enfermedad/viaje)». **Forbidden:** causal certainty («el alcohol TE baja la HRV» without the hedge), population/clinical claims, a p-value as headline, deltas without «~», the word «descubrimos». The three result strings enter this allow-list before they surface.
