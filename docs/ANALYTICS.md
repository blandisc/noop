...
**One source per session, never a sum:** a rating (the one the user answers in the receipt — the per-set suggestion is never written as an answer) → `.rpe`; else a covered pulse → `.hr` (the number this path always stored); else `nil`. The pulse keeps feeding `avgHr` and the cardiovascular recovery cost in every case.
