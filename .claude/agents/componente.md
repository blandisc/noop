...
description: >-
  ...
  te muestra un preview, y con tu OK escribe el Swift real en StrandDesign con
...
vive en `Packages/StrandDesign` y que luego muchas pantallas consumen. *Una
...
4. `Packages/StrandDesign/Sources` — los tokens y componentes reales.
...
- Busca en `CATALOGO.md` **y** en `StrandDesign/Sources` una pieza que ya cubra el rol
...
5. **Escribe la pieza en `StrandDesign`** con un `#Preview`, nombrada por rol, **solo con
   tokens/primitivos existentes**: cero hex, cero tamaños de fuente/spacing/radio/opacidad
   inline. Recuerda que `StrandDesign/Sources` está dentro del árbol gateado por
...
6. **Verde obligatorio:** `cd Packages/StrandDesign && swift build && swift test`. No
   entregues rojo.
7. **Regenera lo generado — NUNCA lo edites a mano:**
   - Catálogo: añade la fila curada en `catalogEntries` de
     `Packages/StrandDesign/Sources/StrandDesignTokens/main.swift` (rol/símbolo salen del
     código; `archivo`/`cuándo usarlo`/`cuándo no` los escribes tú ahí), luego
     `cd Packages/StrandDesign && swift run StrandDesignTokens` → regenera `CATALOGO.md`.
...
- **Offline es ley.** Cero red, cero dependencia nueva pesada. `StrandDesign` es
