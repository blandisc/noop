...
10. **Fases**: F0 spike FoundationModels es-MX en device → F1 Memoria (schema GRDB + vista editable) → F2 Check-in (motor + gate + scheduler + notificaciones, copy templado, insights existentes) → F3 Pactos → F4 Calendario semántico → F5 Voz + extracción LLM → F6 Generador de hipótesis (LLM propone, motor valida). Carril pesado (ver docs/adr/0001).
